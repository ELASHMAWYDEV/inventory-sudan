...
class _FarmToDryingScreenState extends State<FarmToDryingScreen> {
  ...
  List<Map<String, dynamic>> _farmToDryingData = [];
  late final Stream<List<Map<String, dynamic>>> _dataStream;

  @override
  void initState() {
    super.initState();
    _dataStream = _dataService.farmToDryingStream.map((data) => data.map((record) => record.toMap()).toList());
    _loadData();
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      final data = await _dataService.getFarmToDryingRecords();
      setState(() {
        _farmToDryingData = data.map((record) => record.toMap()).toList();
        _isLoading = false;
      });
    } catch (e) {
